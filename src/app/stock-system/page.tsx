            ...
            try {
                const updatedProducts = await Promise.all(
                    products.map(async (product) => {
                        ...
                        const response = await fetch(`http://localhost:8888/v1/stocks/getPicsByProductId/${product.productId}`);
                        if (!response.ok) throw new Error(`Failed to fetch for product ${product.productId}`);
                        const imageData = await response.json();
                        const primaryImage = imageData.images.find((img: any) => img.isPrimary);

                        return {
                            ...product,
                            imageUrl: primaryImage
                                ? `${primaryImage.data}`
                                : "", // Default to empty if no primary image
                        };
                    })
                );
