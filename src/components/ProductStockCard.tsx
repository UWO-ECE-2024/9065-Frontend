...
const ProductStockCard: React.FC<ProductStockCardProps> = (props) => {
                ...
                if (response.ok) {
                    ...
                    if (props.productCategory) {
                        const category = data.data.find((cat: Category) => 
                            cat.categoryId === props.productCategory
                        );
    ...
    useEffect(() => {
        const loadProductImage = async () => {
            try {
                const response = await fetch(`${API_URL}/v1/stocks/getPicsByProductId/${props.productId}`);
                if (response.ok) {
                    const data = await response.json();
                    const primaryImage = data.images.find((img: any) => img.isPrimary);
                    if (primaryImage) {
                        setPreviewImage(`data:${primaryImage.mimeType};base64,${primaryImage.data}`);
                        setFormData(prev => ({
                            ...prev,
    ...
    const handleDelete = async () => {
        ...
        if (isConfirmed) {
            try {
                const response = await fetch(`${API_URL}/v1/stocks/deleteProductById/${props.productId}`, {
                    method: 'PATCH',
                });
                if (response.ok) {
                    props.onStockChange?.();
    ...
    const handleEdit = async (e: React.FormEvent) => {
        ...
        try {
            const category = categories.find(c => c.name === formData.productCategory);
            if (!category) {
    ...
    const handleIncreaseStock = async () => {
        try {
            const response = await fetch(`${API_URL}/v1/stocks/increaseStockById/${props.productId}`, {
                method: 'PATCH'
            });
    ...
    const handleDecreaseStock = async () => {
        try {
            const response = await fetch(`${API_URL}/v1/stocks/decreaseStockById/${props.productId}`, {
                method: 'PATCH'
            });
            ...
            <Card className="hover:shadow-lg duration-200 ease-in-out">
                <CardHeader>
                     <CardTitle><Button 
                         ...
                         🗑️
                     </Button>  {formData.name} </CardTitle>
                </CardHeader>
                ...
                <CardContent>
                    ...
                    <p className="text-xl">Category: {formData.productCategory || "No Category"}</p>
